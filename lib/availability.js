...
export function getFacilityOptions() {
  return lacityFacilities.map(({ id, name, location, neighborhood }) => ({ id, name, location, neighborhood }));
}

export async function getAvailability({ date, area } = {}) {
  try {
    const { slots: liveSlots, errors } = await fetchLacityWebtracAvailability({ date: dateFromIso(date), area });
    const liveSources = [{ ...liveSource, lastChecked: new Date().toISOString() }];
    return {
      mode: "live",
      searchDate: date,
      searchArea: area,
      ...
      warnings: errors,
      sources: liveSources,
      slots: normalizeSlots(liveSlots, liveSources)
    };
  } catch (error) {
    return {
