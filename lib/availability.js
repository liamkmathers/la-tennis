...
import {
  fetchSantaMonicaActiveNetAvailability,
  santaMonicaFacilities,
  santaMonicaSourceId
} from "./providers/santaMonicaActiveNet";
...
const santaMonicaSource = {
  id: santaMonicaSourceId,
  name: "Santa Monica ActiveNet",
  status: "live",
  lastChecked: null
};
...
export function getFacilityOptions() {
  return [...lacityFacilities, ...triFitFacilities, ...santaMonicaFacilities].map(
    ({ id, name, location, neighborhood }) => ({
      id,
      name,
      location,
      neighborhood
...
export async function getAvailability({ date, area } = {}) {
  try {
    ...
    const [lacitySettled, triFitSettled, santaMonicaSettled] = await Promise.allSettled([
      fetchLacityWebtracAvailability({ date: searchDate, area }),
      fetchTriFitAvailability({ date: searchDate, area }),
      fetchSantaMonicaActiveNetAvailability({ date: searchDate, area })
    ]);
    const lacityResult =
    ...
    const santaMonicaResult =
      santaMonicaSettled.status === "fulfilled"
        ? santaMonicaSettled.value
        : { slots: [], errors: [`Santa Monica ActiveNet: ${santaMonicaSettled.reason.message}`] };
    const checkedAt = new Date().toISOString();
    const liveSources = [
      { ...liveSource, lastChecked: checkedAt },
      { ...triFitSource, lastChecked: checkedAt },
      { ...santaMonicaSource, lastChecked: checkedAt }
    ];
    return {
      mode: "live",
      searchDate: date,
      ...
      warnings: [...lacityResult.errors, ...triFitResult.errors, ...santaMonicaResult.errors],
      sources: liveSources,
      slots: normalizeSlots(
        [...lacityResult.slots, ...triFitResult.slots, ...santaMonicaResult.slots],
        liveSources
      )
    };
  } catch (error) {
